...
enum AuthStep { requestOtp, verifyOtp, newUserOtp, register, registerVerifyOtp }
...
class _AuthScreenState extends State<AuthScreen> with TickerProviderStateMixin {
  ...
  Future<void> _handleRequestOtp() async {
    ...
    if (auth.state == AuthState.otpSent) {
      ...
      if (err.toLowerCase().contains('not found') ||
          ...
          err.toLowerCase().contains("doesn't exist")) {
        _registerMobileController.text = mobile;
        setState(() => _currentStep = AuthStep.newUserOtp);
      } else {
        _showSnack(err);
      }
    }
  ...
  Future<void> _handleVerifyNewUserOtp() async {
...
Future<void> _handleRegister() async {
  ...
  InputDecoration _inputDecoration(String hint, {bool isError = false}) {
    return InputDecoration(
      ...
      focusedBorder: OutlineInputBorder(
        borderRadius: BorderRadius.circular(8),
        borderSide: BorderSide(color: _tan, width: 1.5),
      ),
      filled: true,
      fillColor: isError ? _errorFill : Colors.white,
    );
  ...
  Widget _buildNewUserOtpSheet(AuthProvider auth) {
    final loading = auth.isLoading;
    return Column(
      mainAxisSize: MainAxisSize.min,
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        ...
        const SizedBox(height: 24),
        TextField(
          controller: _newUserOtpController,
          keyboardType: TextInputType.number,
          maxLength: 4,
          textAlign: TextAlign.center,
        ...
        loading
            ? _loadingButton('Verifying...')
            : _primaryButton('Verify & Continue', _handleVerifyNewUserOtp),
        const SizedBox(height: 8),
        Center(
          child: TextButton(
            onPressed: () {
...
//         child: Row(
//           children: [
//             const Icon(Icons.error_outline_rounded, color: Color(0xFFCFB896), size: 18),
//             const SizedBox(width: 10),
//             Expanded(
