  ...
  Future<void> _autoFillFromLocation() async {
    ...
    try {
      ...
      if (permission == LocationPermission.denied ||
        ...
        if (!mounted) return;
        _showSnackBar(context, 'Location permission denied', isError: true);
        return;
      }
      ...
      final response = await http.get(
        url,
        headers: {'User-Agent': 'BrublaApp/1.0'},
      );
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          if (parts.isNotEmpty) _line1Controller.text = parts.join(', ');

          final landmark =
              addr['neighbourhood'] ?? addr['quarter'] ?? addr['county'] ?? '';
          if (landmark.isNotEmpty) _line2Controller.text = landmark;
          ...
          if (postcode.isNotEmpty) {
            _pincodeController.text = postcode
                ...
                .substring(
                  0,
                  postcode.replaceAll(RegExp(r'\D'), '').length > 6
                      ? 6
                      : postcode.replaceAll(RegExp(r'\D'), '').length,
                );
          }
        });
      ...
      if (!mounted) return;
      _showSnackBar(context, 'Location error: ${e.toString()}', isError: true);
    } finally {
        ...
        child: Column(
          ...
          children: [
            ...
            _UseMyLocationButton(
              ...
              muted: _muted,
              border: _border,
              onTap: _isLocating ? null : _autoFillFromLocation,
            ),
                ...
                children: [
                  AnimatedContainer(
                    ...
                    decoration: BoxDecoration(
                      ...
                      border: Border.all(
                        color: _isDefault ? _accent : _border,
                        width: 2,
                      ),
                    ),
                    child: _isDefault
...
class _UseMyLocationButton extends StatelessWidget {
  ...
  final Color accent, accentLight, ink, muted, border;
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          color: isLocating ? accentLight.withOpacity(0.5) : accentLight,
          borderRadius: BorderRadius.circular(12),
          border: Border.all(
            color: isLocating
        ...
        child: Row(
          children: [
            AnimatedContainer(
              ...
              height: 38,
              decoration: BoxDecoration(
                color: isLocating ? accent.withOpacity(0.15) : accent,
...
//        required this.selectionMode, // NEW
...
class _AddressCard extends StatelessWidget {
  ...
  final bool isLoading;
  final bool selectionMode;
  final VoidCallback onEdit, onDelete, onSetDefault, onSelect;
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: [
            BoxShadow(
              ...
              blurRadius: selectionMode ? 16 : 12,
              offset: const Offset(0, 3),
            ),
          ],
        ),
                      ...
                      children: [
                        ...
                        Row(
                          children: [
                            ...
                            const Spacer(),
                            // "Tap to select" badge — only in selection mode
                            if (selectionMode)
                              Container(
                                padding: const EdgeInsets.symmetric(
                                  horizontal: 8,
